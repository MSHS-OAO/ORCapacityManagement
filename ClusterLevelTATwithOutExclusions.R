...
summary_used <- schedule_data_needed_cols %>%
  group_by(HOSPITAL,
           Weekday,
           SURGERY_DATE)%>%
  summarise(ProcessTimeMin = (sum(overlap_duration_seconds)/60)+(sum(overlap_setup_seconds,na.rm = TRUE)/60))
...
summary_used_prime_time <- summary_used_prime_time %>%
  select(HOSPITAL,SURGERY_DATE,UtilizationNormalizedPrimeTimeCascade,UtilizationNormalizedPrimeTimeAllOpen) %>%
  pivot_longer( cols = c(`UtilizationNormalizedPrimeTimeCascade`, `UtilizationNormalizedPrimeTimeAllOpen`),
...
get_plot <- function(SITE){
  plot_box <- ggplot(summary_used_prime_time %>% filter(HOSPITAL == SITE), aes(y = AllOpenvsCascade, x = Utilization,fill = AllOpenvsCascade)) +
    ...
    geom_vline(xintercept = 0.70, linetype = "dashed", color = "red")
}
...
ggsave("MSB_MSBI_with_TAT.png",MSB | MSBI,width = 2705, height = 1709, units = "px")
# ggsave("MSBI_with_TAT.pdf",MSBI)
# ggsave("MSH_with_TAT.pdf",MSH)
# ggsave("MSM_with_TAT.pdf",MSM)
ggsave("MSH_MSM_with_TAT.png",MSH|MSM,width = 2705, height = 1709, units = "px")
ggsave("MSQ_MSW_with_TAT.png",MSQ | MSW,width = 2705, height = 1709, units = "px")
